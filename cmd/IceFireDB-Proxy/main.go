...
func main() {
	app := cli.NewApp()
	app.Name = "IceFireDB-Proxy"
	app.Description = "IceFireDB proxy, easier to use IceFireDB."
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:     "config,c",
			Usage:    "config file",
...
func start(c *cli.Context) error {
	...
	for sig := range sigs {
		...
		case syscall.SIGQUIT, syscall.SIGTERM, syscall.SIGINT:
			...
			select {
			case <-ok:
				logrus.Info("shutdown IceFireDB-Proxy.")
...
func showBanner() {
	logo := `  _____           ___ _             ___  ___         ___                     
  \_   \___ ___  / __(_)_ __ ___   /   \/ __\       / _ \_ __ _____  ___   _ 
   / /\/ __/ _ \/ _\ | | '__/ _ \ / /\ /__\//_____ / /_)/ '__/ _ \ \/ / | | |
/\/ /_| (_|  __/ /   | | | |  __// /_// \/  \_____/ ___/| | | (_) >  <| |_| |
\____/ \___\___\/    |_|_|  \___/___,'\_____/     \/    |_|  \___/_/\_\\__, |
                                                                       |___/ 
`
	fmt.Println(logo)
